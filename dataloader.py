...
def collate_fn(batch):
    ...
    max_id = max([len(id)  for f in batch for id in f["label_id"]])
    input_ids = [f["input_ids"] + [0] * (max_len - len(f["input_ids"])) for f in batch]
    input_mask = [[1.0] * len(f["input_ids"]) + [0.0] * (max_len - len(f["input_ids"])) for f in batch]
    labels = [f["labels"] for f in batch]
    entity_pos = [f["entity_pos"] for f in batch]
    hts = [f["hts"] for f in batch]
    label_id = [id + [0] * (max_id - len(id)) for f in batch for id in f["label_id"]]
    # sents = [f["sents"] for f in batch]
    input_ids = torch.tensor(input_ids, dtype=torch.long)
    input_mask = torch.tensor(input_mask, dtype=torch.float)
    label_id = torch.tensor(label_id, dtype=torch.long)
    output = (input_ids, input_mask, labels, entity_pos, hts,label_id)
    return output    